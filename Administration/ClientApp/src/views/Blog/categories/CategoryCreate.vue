<template>
  ...
  <div class="p-4 border font-medium text mb-4">
    <button type="submit" class="border rounded py-1 px-3 bg-gray-500 hover:bg-gray-700 text-white" @click="createModel(model)">Создать</button>
  </div>
</template>
...
<script setup>
  ...
  const createModel = async (model) => {
    ...
    try {
      const data = await apiService.sendDataType('Create', 'blog_ctg', model, 1);

      if (data.success) {
        toast.update(pendingToastId, {
          render: 'Запись успешно создана 👌',
          type: 'success',
          isLoading: false,
          autoClose: 2000
        });
        ...
        throw new Error('Ошибка при создании категории блога');
      }
      ...
      toast.update(pendingToastId, {
        render: 'Ошибка при создании модели 🤯',
        type: 'error',
        isLoading: false,
        autoClose: 2000
      });
