<template>
  ...
  <div class="p-4 border font-normal text mb-4">
    ...
    <div class="grid grid-cols-1 mb-4">
      ...
      <input type="text" v-model="bodyBlog.title" class="border h-9 pl-3 rounded text-sm" />
    </div>
    <div class="grid grid-cols-1 mb-4">
      <label class="text-sm">ЧПУ</label>
      <input type="text" v-model="bodyBlog.sefname" class="border h-9 pl-3 rounded text-sm" />
    </div>
    <div class="grid grid-cols-1 mb-4">
      <label class="text-sm">Дата публикации</label>
      <DatePicker class="custom-datepicker" v-model="bodyBlog.publish_date" :format="format" auto-apply partial-flow :flow="['calendar', 'time']" />
    </div>
    <div class="grid grid-cols-1 mb-4">
      <label class="text-sm">Описание</label>
      <textarea class="border pt-1 pl-3 h-auto rounded text-sm" v-model="bodyBlog.description"></textarea>
    </div>
    <div class="grid grid-cols-1 mb-4">
      <label class="text-sm">Текст</label>
      <quill-editor-vue v-model="state.content" :disabled="state.disabled" />
    </div>
...
<script setup>
  ...
  const cropImage = () => {
    if (crop.value) {
      ...
      if (canvas) {
        const img = new Image();
        const croppedImage = canvas.toDataURL(imgObj.imgType);
        img.onload = () => {
          imgObj.imgSrc = croppedImage;
          imgObj.imgWidth = img.width;
          imgObj.imgHeight = img.height;
        }
