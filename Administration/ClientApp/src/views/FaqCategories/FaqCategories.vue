<template>
  <div class="p-4 border">
    ...
    <div class="flex flex-col">
      ...
      <div v-for="(item, index) in items" :key="item.id" class="grid grid-cols-6 gap-0" style="grid-template-columns: 0.5fr 6fr 2fr 2fr 2fr">
        ...
        <div class="p-2 border">
          <input type="checkbox" :checked="item.is_active === 1" @change="handleCheckboxChange(item)" />
        </div>
        <div class="p-2 border">
          ...
          <button @click="goToPageById('FaqCategoriesEdit',item.id)" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4">
    ...
    <FaqCategoriesEdit v-if="showForm"
                  :item="selectedItem"
                  @save="saveItem"
                  @cancel="cancelForm" />
...
<script setup>
  ...
  onMounted(async () => {
    try {
      ...
      const data = await apiService.fetchDataByTypeLang('Index', 'faq_category', 1); 
  ...
  */
  function goToPageById(url, id) {
    //router.push(`${url}/${id}`);
    console.log('FaqCategory goToPageById id=', id);
    router.push({ name: url, params: { id: id } });
  }

  function handleCheckboxChange(item) {
      // Присваиваем числовое значение 1 если чекбокс отмечен, и 0 если нет
  ...
  async function ChangeData(item) {
    try {
      ...
      await apiService.sendData(`Update?type=faq_category&id=${item.id}`, item, 'post')
    } catch (e) {
